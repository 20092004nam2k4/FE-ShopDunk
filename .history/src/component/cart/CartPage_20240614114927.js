...
const CartPage = () => {
  ...
  const updateQuantity = (id, updateQuantityuantity) => {
    // Đảm bảo newQuantity không âm hoặc bằng không
    if (newQuantity <= 0) {
      // Ví dụ: Xoá sản phẩm khỏi giỏ hàng khi số lượng bằng 0
      removeFromCart(id);
      return;
    }
    ...
    axios
      ...
      .then(() => {
        // Sau khi cập nhật số lượng thành công, làm mới giỏ hàng
        fetchCartItems();
      })
      .catch((error) => {
                    ...
                    <td className="quantity">
                      <div className="cart-quantity-input-container">
                        <svg
                          ...
                          onClick={() => updateQuantity(item.id, item.quantity - 1)} // Giảm số lượng
                        >
                          <path
                        ...
                        <svg
                          ...
                          fill="none"
                          xmlns="http://www.w3.org/2000/svg"
                          onClick={() => updateQuantity(item.id, item.quantity + 1)} // Tăng số lượng
                        >
                          <path
