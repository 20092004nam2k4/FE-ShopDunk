...
const ProductDetail = () => {
  ...
  // If user is not logged in and sessionId is not set, generate a new sessionId
  if (!userName) {
    if (!sessionId) {
    ...
    sessionId = null; // If user is logged in, sessionId is not needed
  }
  ...
  const addToCart = () => {
    ...
    axios
      .post(`http://localhost:8090/api/products/addToCart/${product.id}`, null, {
        params: {
          sessionId,
          userName,
          quantity: 1
        ...
        <div className="product-detail-info">
          ...
          <div className="star-rating">
            {[...Array(5)].map((star, index) => {
              const ratingValue = index + 1;<p className="star-color">đánh giá</p>
              return (
                <FaStar
                  key={index}
                  className="star"
